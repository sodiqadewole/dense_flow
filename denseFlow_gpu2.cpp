...
// Convert flow vectors to flow image with three channels (flow_x,flow_y,mag)
// Added on 19 Jan 2017 by Kaiyang Zhou
static void cvtFlowTo3CImg(const Mat &flow_x, const Mat &flow_y, Mat &flow_img) {
  for (int i = 0; i < flow_x.rows; ++i) {
		for (int j = 0; j < flow_y.cols; ++j) {
			float x = flow_x.at<float>(i,j);
			float y = flow_y.at<float>(i,j);
      ...
      x = x * 16 + 128; if (x > 255) {x = 255;} else if (x < 0) {x = 0;}
      y = y * 16 + 128; if (y > 255) {y = 255;} else if (y < 0) {y = 0;}
      m = m * 16 + 128; if (m > 255) {m = 255;} else if (m < 0) {m = 0;}

      flow_img.at<Vec3b>(i,j)[0] = x;
      flow_img.at<Vec3b>(i,j)[1] = y;
      flow_img.at<Vec3b>(i,j)[2] = m;
		}
	}
}
...
int main(int argc, char** argv){
    ...
    cvtFlowTo3CImg(flow_x, flow_y, flow_img);

		char tmp[20];
		//sprintf(tmp,"_%05d.jpg",int(frame_num));
    sprintf(tmp,"flow_image_%04d.jpg",int(frame_num));
